...
import { calculateLayout, drawBoard, drawFallingTiles, drawJumperFlights, } from "../render/board-renderer";
...
import { ALL_TILE_THEME_DEFINITIONS, getTextureKeyForThemeTile, getThemeForLevel, } from "../themes/tile-themes";
import { SPECIAL_TEXTURE_KEYS } from "../themes/special-assets";
import { BG_COLOR, PATH_DISPLAY_DURATION, OVERLAY_BG, } from "../constants";
import { AlbumProgressStore, XP_PER_LEVEL_CLEAR, XP_PER_PHOTO_UNLOCK, PHOTO_ALBUM_SIZE, getAllPhotoIdsSorted, getPhotoAssetPath, } from "../album/photo-album";
...
const RANDOM_GRAVITY_DIRECTIONS = [
    "up",
    "down",
    "left",
    "right",
...
const TUTORIAL_SCRIPT = [
    ...
    {
        id: "hud-buttons",
        message: "Quick controls: Home returns to main menu, Hint reveals a match, Replay restarts the level, and Settings opens options.",
        placement: "bottom",
        tapToContinue: true,
...
export class GameScene extends Phaser.Scene {
    constructor() {
        ...
        this.renderPixelRatio = 1;
        this.tileImagesByTextureKey = new Map();
        this.activeTheme = "foods";
        ...
        this.progression = new LevelProgression(LEVELS, START_LEVEL_ID);
        ...
        this.tutorialActiveBoard = "intro";
        ...
        this.resolveTileImage = (tileType) => {
    ...
    create() {
        ...
        this.levelProgressStore = new LevelProgressStore(this.progression.getTotalLevels(), START_LEVEL_ID);
        ...
        this.settingsModal = new SettingsModal(this.settings, (next) => {
        ...
        this.startScreen = new StartScreen(() => this.handleStartScreenPlay(), () => this.handleStartScreenSettings(), (levelId) => this.handleStartScreenLevelSelect(levelId), () => this.handleStartScreenTutorial(), () => this.audio.play(GAME_SOUNDS.BUTTON_CLICK_PRIMARY));
    ...
    initLevel(def, preserveRunScore, persistLastPlayed = true) {
    ...
    ensureBoardHasValidMove(board, playShuffleSound) {
    ...
    recalculateLayout() {
        this.layout = calculateLayout(this.displayWidth, this.displayHeight, this.gameState.board.width, this.gameState.board.height);
    }
    ...
    hydrateThemeTileImages() {
        ...
        if (this.textures.exists(SPECIAL_TEXTURE_KEYS.monkey)) {
            ...
            this.monkeyImage = monkeyTexture.getSourceImage() ?? null;
        }
        if (this.textures.exists(SPECIAL_TEXTURE_KEYS.iceOverlay)) {
            ...
            this.iceOverlayImage = iceTexture.getSourceImage() ?? null;
        }
        if (this.textures.exists(SPECIAL_TEXTURE_KEYS.foodsBackground)) {
            ...
            this.foodsBackgroundImage =
                foodsBackgroundTexture.getSourceImage() ?? null;
        }
        if (this.textures.exists(SPECIAL_TEXTURE_KEYS.landmarksBackground)) {
            ...
            this.landmarksBackgroundImage =
                landmarksBackgroundTexture.getSourceImage() ?? null;
        }
        if (this.textures.exists(SPECIAL_TEXTURE_KEYS.planetsBackground)) {
            ...
            this.planetsBackgroundImage =
                planetsBackgroundTexture.getSourceImage() ?? null;
        }
    }
    updateState(dt) {
    ...
    handleTap(coord, boardWidth, boardHeight) {
        if (this.startScreen.isVisible()) {
            return;
        }
        if (this.settingsOpen) {
    ...
    showHintFeedback(text, seconds) {
        this.hintFeedbackText = text;
        this.hintFeedbackTimer = seconds;
    }
    startMatchClearAnimationForPendingRemoval(path) {
        ...
        if (firstCell.kind !== CellKind.Tile ||
    ...
    updateMatchClearAnimation(dt, deltaMs) {
    ...
    samplePathPoint(points, segmentLengths, distance) {
        if (points.length === 0) {
            return { x: 0, y: 0 };
        }
    ...
    samplePathTangent(points, segmentLengths, distance) {
    ...
    getHiddenBoardCoordsForMatchClear() {
        if (!this.matchClearAnimation) {
            return [];
        }
    ...
    executeRemoval() {
        ...
        const isChainMatch = this.lastSuccessfulMatchAtMs > 0 &&
        ...
        if (remaining === 0) {
            ...
            this.audio.play(this.campaignCompleted ? GAME_SOUNDS.CAMPAIGN_COMPLETE : GAME_SOUNDS.LEVEL_COMPLETE);
    ...
    startMergePullAnimation(a, b, tileType) {
        ...
        if (this.mergePullAnimations.length > GameScene.MAX_MERGE_PULL_ANIMATIONS) {
            this.mergePullAnimations.splice(0, this.mergePullAnimations.length - GameScene.MAX_MERGE_PULL_ANIMATIONS);
        }
    }
    getMergePullRenderItems() {
        if (this.mergePullAnimations.length === 0) {
            return [];
        }
    ...
    startGravitySlideAnimations(moves) {
        if (moves.length === 0) {
            return;
        }
        const additions = [];
        for (const move of moves) {
            ...
            if ((landedCell.kind !== CellKind.Tile && landedCell.kind !== CellKind.FrozenTile) ||
                landedCell.tileType === null) {
                continue;
            }
            const distance = Math.abs(move.from.row - move.to.row) + Math.abs(move.from.col - move.to.col);
            const duration = Math.max(GameScene.GRAVITY_SLIDE_BASE_DURATION_MS, Math.min(GameScene.MAX_GRAVITY_SLIDE_DURATION_MS, GameScene.GRAVITY_SLIDE_BASE_DURATION_MS + distance * GameScene.GRAVITY_SLIDE_PER_CELL_MS));
            additions.push({
                from: { ...move.from },
                to: { ...move.to },
        ...
        if (this.gravitySlideAnimations.length > GameScene.MAX_GRAVITY_SLIDE_ANIMATIONS) {
            this.gravitySlideAnimations.splice(0, this.gravitySlideAnimations.length - GameScene.MAX_GRAVITY_SLIDE_ANIMATIONS);
        }
    }
    getGravitySlideRenderItems() {
        if (this.gravitySlideAnimations.length === 0) {
            return [];
        }
    ...
    shouldUseRandomGravity(def) {
        return def.gravity !== "none";
    }
    rollRandomGravityMatchWindow() {
        const span = GameScene.RANDOM_GRAVITY_MAX_MATCHES - GameScene.RANDOM_GRAVITY_MIN_MATCHES + 1;
        return GameScene.RANDOM_GRAVITY_MIN_MATCHES + Math.floor(Math.random() * span);
    }
    pickNextRandomGravityDirection(current) {
    ...
    advanceRandomGravityAfterMatch() {
    ...
    startJumperJumpAnimations(moves) {
        ...
        for (const move of moves) {
            const distance = Math.abs(move.from.row - move.to.row) + Math.abs(move.from.col - move.to.col);
            const durationMs = Math.max(GameScene.JUMPER_JUMP_BASE_DURATION_MS, Math.min(GameScene.JUMPER_JUMP_MAX_DURATION_MS, GameScene.JUMPER_JUMP_BASE_DURATION_MS + distance * GameScene.JUMPER_JUMP_PER_CELL_MS));
            const arcHeightPx = Math.max(this.layout.cellSize * 0.44, Math.min(this.layout.cellSize * 1.15, this.layout.cellSize * (0.56 + distance * 0.09)));
            additions.push({
                from: { ...move.from },
                to: { ...move.to },
                elapsedMs: 0,
        ...
        if (this.jumperJumpAnimations.length > GameScene.MAX_JUMPER_JUMP_ANIMATIONS) {
            this.jumperJumpAnimations.splice(0, this.jumperJumpAnimations.length - GameScene.MAX_JUMPER_JUMP_ANIMATIONS);
        }
    }
    getJumperJumpRenderItems() {
        if (this.jumperJumpAnimations.length === 0) {
            return [];
        }
    ...
    getHiddenBoardCoordsForJumperJumps() {
        if (this.jumperJumpAnimations.length === 0) {
            return [];
        }
    ...
    renderFrame() {
        ...
        drawBoard(ctx, this.gameState.board, this.layout, this.gameState.selectedTile, pathToDraw, pathAlpha, this.resolveTileImage, this.monkeyImage, this.iceOverlayImage, this.getMergePullRenderItems(), this.getGravitySlideRenderItems(), hiddenCoords, gravityEdgePulse);
        drawJumperFlights(ctx, this.layout, this.layout.cellSize, this.getJumperJumpRenderItems(), this.monkeyImage);
        drawFallingTiles(ctx, this.layout.cellSize, this.getMatchClearRenderItems(), this.resolveTileImage);
        this.drawKeyboardCursor(ctx);
        this.hintButton.setDisabled(this.startScreen.isVisible() ||
            this.settingsOpen ||
        ...
        const tutorialButtonsStepActive = this.tutorialEnabled && this.getCurrentTutorialStep()?.id === "hud-buttons";
        ...
        const shouldShowHintFeedback = !this.startScreen.isVisible() &&
            !this.settingsOpen &&
    ...
    drawSceneBackground(ctx, width, height) {
        ...
        ctx.fillRect(0, 0, width, height);
        const vignette = ctx.createRadialGradient(width * 0.5, height * 0.42, Math.min(width, height) * 0.14, width * 0.5, height * 0.5, Math.max(width, height) * 0.86);
        vignette.addColorStop(0, "rgba(255, 255, 255, 0.04)");
        vignette.addColorStop(1, "rgba(0, 0, 0, 0.18)");
        ctx.fillStyle = vignette;
        ctx.fillRect(0, 0, width, height);
    }
    getThemeBackgroundOverlay() {
    ...
    getCanvasImageSize(image) {
        const candidate = image;
        const width = typeof candidate.videoWidth === "number" && candidate.videoWidth > 0
            ? candidate.videoWidth
            : typeof candidate.width === "number" && candidate.width > 0
                ? candidate.width
                : 0;
        const height = typeof candidate.videoHeight === "number" && candidate.videoHeight > 0
    ...
    handleTutorialPointerDown(pointer) {
        if (!this.tutorialEnabled) {
            return false;
        }
    ...
    createTutorialBoardPreset(kind) {
        if (kind === "gravity") {
            return {
    ...
    makeTutorialBoard(rows, gravity) {
    ...
    rectToTutorialSpotlight(rect, padding = 8, radius = 12) {
        return {
            kind: "rect",
    ...
    drawOverlay(ctx, w, h) {
        ...
        if (this.gameState.phase === "won") {
            ...
            ctx.fillText(`XP ${this.getXpProgressInStep()}/${GameScene.XP_PROGRESS_STEP}`, w / 2, progressY + 20);
    ...
    handleStartScreenSettings() {
        this.audio.play(GAME_SOUNDS.BUTTON_CLICK_PRIMARY);
        this.settingsModal.openFromExternalTrigger();
        this.triggerHaptic("light");
    }
    handleStartScreenTutorial() {
        if (!this.startScreen.isVisible() || this.settingsOpen) {
            return;
        }
    ...
    handleStartScreenLevelSelect(levelId) {
        if (!this.startScreen.isVisible() || this.settingsOpen) {
            return;
        }
    ...
    getUnlockedThroughLevelId() {
        const totalLevels = this.progression.getTotalLevels();
        const baseline = Math.max(1, Math.min(totalLevels, START_LEVEL_ID));
        if (!this.levelProgressStore) {
            return baseline;
        }
        return Math.max(baseline, Math.min(totalLevels, this.levelProgressStore.getHighestUnlockedLevel()));
    }
    getResumeLevelId() {
        const totalLevels = this.progression.getTotalLevels();
        const baseline = Math.max(1, Math.min(totalLevels, START_LEVEL_ID));
        ...
        const highestUnlocked = Math.max(baseline, Math.min(totalLevels, this.levelProgressStore.getHighestUnlockedLevel()));
        const lastPlayed = Math.max(baseline, Math.min(highestUnlocked, this.levelProgressStore.getLastPlayedLevel()));
        return lastPlayed;
    }
    syncStartScreenAlbumState() {
        ...
        const entries = getAllPhotoIdsSorted(PHOTO_ALBUM_SIZE).map((photoId) => ({
            photoId,
    ...
    syncStartScreenLevelSelectionState() {
        if (!this.startScreen) {
            return;
        }
    ...
    findInitialKeyboardCursor(board) {
        for (let row = 0; row < board.height; row++) {
            for (let col = 0; col < board.width; col++) {
                const cell = board.cells[row][col];
                if (cell.kind === CellKind.Tile) {
...
GameScene.SCENE_KEY = "GameScene";
GameScene.MAX_INIT_BUILD_ATTEMPTS = 12;
GameScene.MAX_RESHUFFLE_ATTEMPTS = 24;
GameScene.HINT_DISPLAY_DURATION_MS = 1400;
GameScene.HINT_FEEDBACK_DURATION_S = 1.2;
GameScene.XP_PROGRESS_STEP = XP_PER_PHOTO_UNLOCK;
GameScene.LEVEL_WIN_XP_GAIN = XP_PER_LEVEL_CLEAR;
GameScene.OVERLAY_INTRO_DURATION_S = 0.24;
GameScene.MATCH_CHAIN_WINDOW_MS = 1300;
GameScene.TIME_LOW_WARNING_SECONDS = 10;
GameScene.MERGE_PULL_DURATION_MS = 170;
GameScene.MAX_MERGE_PULL_ANIMATIONS = 10;
GameScene.GRAVITY_SLIDE_BASE_DURATION_MS = 95;
GameScene.GRAVITY_SLIDE_PER_CELL_MS = 34;
GameScene.MAX_GRAVITY_SLIDE_DURATION_MS = 220;
GameScene.MAX_GRAVITY_SLIDE_ANIMATIONS = 64;
GameScene.JUMPER_JUMP_BASE_DURATION_MS = 270;
GameScene.JUMPER_JUMP_PER_CELL_MS = 44;
GameScene.JUMPER_JUMP_MAX_DURATION_MS = 520;
GameScene.MAX_JUMPER_JUMP_ANIMATIONS = 24;
GameScene.RANDOM_GRAVITY_MIN_MATCHES = 3;
GameScene.RANDOM_GRAVITY_MAX_MATCHES = 4;
GameScene.GRAVITY_EDGE_PULSE_DURATION_MS = 300;
GameScene.MATCH_CLEAR_APPROACH_DURATION_MS = 220;
GameScene.MATCH_CLEAR_SCATTER_DURATION_MS = 430;
GameScene.MATCH_CLEAR_SCATTER_GRAVITY_PX_S2 = 2750;
GameScene.MATCH_CLEAR_SCATTER_SPEED_PX_S = 300;
GameScene.MATCH_CLEAR_SCATTER_LIFT_PX_S = 245;
