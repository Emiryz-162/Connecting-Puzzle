...
export class BootScene extends Phaser.Scene {
    ...
    preload() {
        ...
        if (!this.textures.exists(SPECIAL_TEXTURE_KEYS.iceOverlay)) {
            this.load.image(SPECIAL_TEXTURE_KEYS.iceOverlay, SPECIAL_ASSET_PATHS[SPECIAL_TEXTURE_KEYS.iceOverlay]);
        }
        if (!this.textures.exists(SPECIAL_TEXTURE_KEYS.foodsBackground)) {
            this.load.image(SPECIAL_TEXTURE_KEYS.foodsBackground, SPECIAL_ASSET_PATHS[SPECIAL_TEXTURE_KEYS.foodsBackground]);
        }
        if (!this.textures.exists(SPECIAL_TEXTURE_KEYS.landmarksBackground)) {
            this.load.image(SPECIAL_TEXTURE_KEYS.landmarksBackground, SPECIAL_ASSET_PATHS[SPECIAL_TEXTURE_KEYS.landmarksBackground]);
        }
        if (!this.textures.exists(SPECIAL_TEXTURE_KEYS.planetsBackground)) {
            this.load.image(SPECIAL_TEXTURE_KEYS.planetsBackground, SPECIAL_ASSET_PATHS[SPECIAL_TEXTURE_KEYS.planetsBackground]);
        }
    }
    create() {
