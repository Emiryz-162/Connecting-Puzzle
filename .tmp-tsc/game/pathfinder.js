...
export function findPath(board, a, b) {
    ...
    // 1-turn paths
    const corner1 = { col: a.col, row: b.row };
    const corner2 = { col: b.col, row: a.row };
    if (isWalkable(board, corner1) &&
        canWalkStraight(board, a, corner1) &&
        canWalkStraight(board, corner1, b)) {
        candidates.push([a, corner1, b]);
    }
    if (isWalkable(board, corner2) &&
    ...
    for (let r = -1; r <= board.height; r++) {
        const c1 = { col: a.col, row: r };
        const c2 = { col: b.col, row: r };
        if (isWalkable(board, c1) &&
            isWalkable(board, c2) &&
    ...
    for (let c = -1; c <= board.width; c++) {
        const c1 = { col: c, row: a.row };
        const c2 = { col: c, row: b.row };
        if (isWalkable(board, c1) &&
            isWalkable(board, c2) &&
            canWalkStraight(board, a, c1) &&
            canWalkStraight(board, c1, c2) &&
...
function canWalkStraight(board, from, to) {
    if (from.row === to.row) {
        const row = from.row;
...
function isWalkable(board, coord) {
    if (coord.row < 0 ||
...
function isCellWalkable(board, row, col) {
    if (row < 0 || row >= board.height || col < 0 || col >= board.width) {
        return true;
    }
    return board.cells[row][col].kind === CellKind.Empty;
}
function pickBestPath(board, candidates) {
...
export function hasAnyValidPair(board) {
