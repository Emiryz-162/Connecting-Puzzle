...
export interface TutorialRectSpotlight {
  kind: "rect";
  x: number;
  y: number;
  width: number;
...
export interface TutorialCircleSpotlight {
  kind: "circle";
  x: number;
  y: number;
  radius: number;
...
export type TutorialSpotlight = TutorialRectSpotlight | TutorialCircleSpotlight;
...
export class TutorialOverlay {
  ...
  private readonly onWindowResize = (): void => this.redraw();
  private state: TutorialOverlayState = {
    visible: false,
    message: "",
    tapHint: "",
    placement: "bottom",
  ...
  setState(next: TutorialOverlayState): void {
    this.state = {
  ...
  private applyCardPlacement(): void {
    ...
    const clampTopCss = (preferredTop: number): string => {
