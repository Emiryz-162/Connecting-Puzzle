...
import { BoardState, CellKind, Coord } from "../types";

/** Jumping blocker hareket bilgisi (animasyon için) */
 ...
 */
export function moveJumpingBlockers(board: BoardState): JumpMove[] {
  ...
  if (jumpers.length === 0) return moves;
  ...
  for (const jumper of jumpers) {
    ...
    if (emptyCells.length === 0) continue; // Boş hücre yoksa atlat
