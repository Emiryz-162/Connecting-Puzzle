...
import { BoardState, Cell, CellKind, Coord, SettleMove } from "../types";
 ...
 */
export function resolveGravity(board: BoardState): SettleMove[] {
...
function packSegment(
  ...
  const toCoord = (s: number): Coord =>
    isVertical ? { row: s, col: p } : { row: p, col: s };
  ...
  if (tiles.length === 0 || tiles.length === segLen) return;
